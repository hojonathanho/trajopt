...
Vector3d quatLog(const Vector4d& q) {
  ...
  double s = q(0);
  double vnorm = v.norm();
  if (vnorm > 0) return (acos(s) / vnorm) * v;
...
class ConstAngVelConstraintND : public ConstraintFromNumDiff {
  struct ErrCalc : public VectorOfVector {
    ...
    ErrCalc(ConstAngVelConstraintND *c) : m_c(c) { }
    VectorXd operator()(const VectorXd& x) const {
      DblMatrix rvals = getTraj(toDblVec(x), m_c->r_);
      DblMatrix qnew(m_c->q_.rows(), m_c->q_.cols());
      for (int i=0; i < qnew.rows(); ++i) {
        qnew.row(i) = quatMult(m_c->q_.row(i), quatExp(rvals.row(i)));
      }
      DblMatrix wvals = getW(qnew, m_c->dt_);

      VectorXd err(3*wvals.rows());
      for (int i = 0; i < wvals.rows(); ++i) {
        for (int j = 0; j < 3; ++j) {
          err[i*3 + j] = wvals(i,j) - m_c->m_w_desired(j);
        }
      }
      return err;
    }
...
public:
  ConstAngVelConstraintND(const DblMatrix& q, VarArray& r, const Vector3d &w_desired, double dt)
    : q_(q), r_(r), dt_(dt), m_w_desired(w_desired),
      ConstraintFromNumDiff(VectorOfVectorPtr(new ErrCalc(this)), r.flatten(), EQ, "blah") {}
  const DblMatrix& q_;
  VarArray r_;
  double dt_;
...
public:
  ...
  vector<double> value(const DblVec& x) {
    DblMatrix rvals = getTraj(x, r_);
    DblMatrix qnew(q_.rows(), q_.cols());
    for (int i=0; i < qnew.rows(); ++i) {
      qnew.row(i) = quatMult(q_.row(i), quatExp(rvals.row(i)));
    ...
    DblMatrix wvals = getW(qnew, dt_);
    vector<double> err(3*wvals.rows(), 0);
    for (int i = 0; i < wvals.rows(); ++i) {
      for (int j = 0; j < 3; ++j) {
        err[i*3 + j] = wvals(i,j) - m_w_desired(j);
      }
    }
    return err;
  }
...
int main(int argc, char** argv) {
  ...
  DblMatrix initquats(n_steps, 4);
  for (int i = 0; i < n_steps; ++i) {
    initquats.row(i) = toVector4d(qstartrave);
  }
  cout << "slerp traj: " << endl << slerptraj << endl;
  cout << "init traj: " << endl << initquats << endl;

  Vector3d const_w(0, 0, 1); Vector4d tmp_q = initquats.row(0);
  for (int i = 0; i < n_steps; ++i) {
    cout << "tmp_q " << i << ' ' << tmp_q.transpose() << endl;
    tmp_q = quatMult(tmp_q, quatExp(const_w*dt));
  }

  QuatTrajProbPtr prob(new QuatTrajProb(initquats, n_steps, dt));
  prob->setIncrementMask(vector<bool>(prob->getNumVars(), true));
  //prob->addCost(CostPtr(new AngVelCost(prob->q_, prob->r_, dt)));
  prob->addCost(CostPtr(new ZeroCost()));
  prob->addConstr(ConstraintPtr(new ConstAngVelConstraint(prob->q_, prob->r_, const_w, dt)));
  ...
  cout << "r:\n" << trajopt::getTraj(opt.results().x, prob->r_) << endl;
  cout << "q:\n" << prob->q_ << endl;
  cout << "w:\n" << getW(prob->q_, dt) << endl;
